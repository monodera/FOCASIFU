...
def bias_subtraction(inhdl):
    print('\t Bias subtracting for the frame ID, '+
          str(inhdl[0].header['FRAMEID']))
    # Bias subtraction
    ...
    bias_template_hdl = fits.open(fi.bias_template_file+str(binfac1)+\
                                  str(detid)+'.fits')
    ...
    #bias_template_data = fits.getdata(fi.bias_template_file+str(detid)+'.fits')
...
def remove_overscan(inhdl):
    print('\t Overscan region removing for '+ str(inhdl[0].header['FRAMEID']))
    ...
    # electron/ADU
    gain = [2.054, 1.987, 1.999, 1.918, 2.081, 2.047, 2.111, 2.087] 
...
def bias_overscan(ifname, rawdatadir='', overwrite = False):
    print('\n#############################')
